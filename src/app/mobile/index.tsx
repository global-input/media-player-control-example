...
export interface MobileInputData extends globalInput.GlobalInputData {
    ControlledContainer: React.FC<ControlledContainerProps>;
    pairing: React.ReactNode;
    disconnectButton: React.ReactNode;
    sendFormFields: (title: string, fields: globalInput.FormField[], domain?: string, formId?: string,dataType?:string, initDataId?:string) => void;
    setOnFieldChange: (onFieldChange: (field: globalInput.FormField) => void) => void
}

const buildInitData = (title: string, fields: globalInput.FormField[], domain?: string, formId?: string,dataType?:string, initDataId?:string):globalInput.InitData=>{
...
export const useMobile = (title: string, fields: globalInput.FormField[] | (() => globalInput.FormField[]), domain?: string, formId?: string,dataType?:string, initDataId?:string): MobileInputData => {
    ...
    const initData=buildInitData(title, fields, domain, formId,dataType, initDataId);

    const mobile = globalInput.useGlobalInputApp({
        initData, options, codeAES: connectionSettings.codeKey
    });

    const sendFormFields = useCallback((title: string, fields: globalInput.FormField[],domain?:string, formId?:string,dataType?:string, initDataId?:string) => {
        const initData=buildInitData(title, fields, domain, formId,dataType, initDataId);
